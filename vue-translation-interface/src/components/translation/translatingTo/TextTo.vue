...
const currentIndex = computed(() => store.indexOfString(props.string.id) + 1)

const pluralized = computed(() => props.string.original_text.pluralized)

const texts = computed(() => {
  console.info(props.string)
  return props.string.translated_text ? props.string.translated_text.raw_text : {}
})

const specialTokens = computed<string[]>(() => {
  const originalTexts = Object.values(props.string.original_text.text)
...
const saveTranslation = async (data: TranslationData) => {
  ...
  loading.value = true
  error.value = null
  const oldPath = props.string.path
  ...
  try {
    ...
    setTimeout(() => {
      if (props.string.path !== oldPath) {
        router.push(translateLink({ dir: props.string.path, params: { force: true } }))
