...
def add_or_update_trstringtext(
    project,
    path,
    name,
    language,
    ...
    if (
        new_string and not editing_original
    ):  # Trying to add translation of non-existing TrString:
        return {"trstring": None, "trstringtext": None}
    ...
    if not new_translation:
        try:
            translated_text = TrStringText.objects.get(
                language=language, trstring=current_string
            )
    ...
    if not editing_original:
        pluralized = TrStringText.objects.get(
            trstring=current_string, language=current_string.project.source_language
        ).pluralized

    parsed_text_data = parse_submitted_text(text, pluralized, language.nplurals())

    if (
        new_translation
        or parsed_text_data["text"] != translated_text.text
        or (
            editing_original
            and (
                translated_text.pluralized != pluralized
                or current_string.context != context
            )
        )
        ...
        if not new_translation and parsed_text_data["text"] != translated_text.text:
            ...
            old_version_pluralized = (
                translated_text.number_of_pluralized_texts()
                == translated_text.language.nplurals()
                and translated_text.language.nplurals() > 1
            )
            history = TrStringTextHistory(
        ...
        if not editing_original and state not in [
        ...
        if editing_original:
            ...
            original_text = get_object_or_404(
                TrStringText,
        ...
        if new_string:
            activity.action = (
                ACTION_TYPE_ADD if importing_user is None else ACTION_TYPE_IMPORT
            )
            activity.words = current_string.words
            activity.characters = current_string.characters
        elif new_translation:
            activity.action = (
                ACTION_TYPE_TRANSLATE if importing_user is None else ACTION_TYPE_IMPORT
            )
            activity.words = current_string.words
            activity.characters = current_string.characters
        else:
    ...
    return {"trstring": current_string, "trstringtext": translated_text}
...
def get_recursive_directories(all_strings):
    ...
    for d in unique_paths:
        strings_in_directory = all_strings.filter(path=d).aggregate(
            Count("name"), Sum("words"), Sum("characters")
        )
        if d == "":
            strings_in_children = all_strings.exclude(path="").aggregate(
                Count("name"), Sum("words"), Sum("characters")
            )
        else:
            strings_in_children = all_strings.filter(
                path__startswith=d + "/"
            ).aggregate(Count("name"), Sum("words"), Sum("characters"))
        all_directories[d] = {
            "strings": {
...
def add_recursive_children_directory(path, all_directories):
    ...
    for dir in children:
        ...
        if complete_path != path:
            new_children[dir] = add_recursive_children_directory(
                complete_path, all_directories
            )
...
def get_strings_to_translate(
    ...
    if sort == SORT_STRINGS_BY_NEWEST:
        ...
        strings = sorted(
            strings, key=lambda i: natural_keys(i.name)
    ...
    for trstr in strings:
        trstr.original_text = TrStringText(
            trstring=trstr,
            language=trstr.project.source_language,
            text="",
...
def update_project_count(project):
    trstrings = TrString.objects.filter(project=project).aggregate(
    ...
    project.characters = (
        0 if trstrings["characters__sum"] is None else trstrings["characters__sum"]
    )
    project.save()
...
def update_language_version_count(l):  # l: languageversion
    ...
    outdated = l.outdated_strings = TrStringText.objects.filter(
        trstring__project=l.project,
        language=l.language,
        state=TRANSLATION_STATE_OUTDATED,
    )
    ...
    translated_aggregated = translated.aggregate(
        Sum("trstring__words"), Sum("trstring__characters")
    )
    outdated_aggregated = outdated.aggregate(
        Sum("trstring__words"), Sum("trstring__characters")
    )
...
def get_project_languages_for_user(project, user):
    if is_project_admin(user, project):
        available_languages = (
            Language.objects.filter(code=project.source_language.code)
            | Language.objects.filter(languageversion__project=project)
        ).distinct()
    else:
        available_languages = Language.objects.filter(
            languageversion__project=project, languageversion__translators=user
        )
...
def get_project_language_statistics(project, user):
    ...
    activities = (
        StringActivity.objects.filter(
            trstringtext__trstring__project=project, action=ACTION_TYPE_TRANSLATE
        )
    ...
    for lv in other_available:
        if lv.language.code in all_activities.keys():
            ...
            lv.translator_stats = []
        lv.translation_request_sent = (
            lv.translatorrequest_set.filter(user=user).count() > 0
        )
...
def addible_languages(project):
    ...
    languages = languages.exclude(code=project.source_language.code).exclude(
        languageversion__project=project
    )
...
def filter_by_state(trstrings, current_language, state):
    if state == STATE_FILTER_UNTRANSLATED:
        ...
        return trstrings.filter(
            trstringtext__language=current_language,
        ...
        outdated = trstrings.filter(
            trstringtext__language=current_language,
            trstringtext__state=TRANSLATION_STATE_OUTDATED,
        )
        return (
            untranslated | outdated
        ).distinct()  # Otherwise some strings appear several times, I don't know why
    else:
        return trstrings
...
def filter_by_search(trstrings, current_language, search_string):
    ...
    if search_string == "" or trstrings.count() == 0:
        ...
        in_source_language = trstrings.filter(
            trstringtext__language=trstrings[0].project.source_language,
        ...
        if trstrings[0].project.source_language == current_language:
            ...
            in_current_language = trstrings.filter(
                trstringtext__language=current_language,
                trstringtext__text__icontains=search_string,
            )
            return (
                in_name | in_current_language | in_source_language
            ).distinct()  # Otherwise some strings appear several times
...
def parse_submitted_text(submitted_text, is_pluralized, nplurals):
    ...
    if is_pluralized:
        ...
        text = (
            submitted_strings["text[0]"]
            if "text[0]" in submitted_strings.keys()
            else submitted_text
        )
        text = text.strip()
...
def get_history_comparison(history):
    ...
    for i in range(len(history) - 1):
        ...
        if len(new_texts) == 1 and len(old_texts) == 1:  # Non-pluralized string
            new.comparison["1"] = get_text_difference(
                html.escape(list(old_texts.values())[0]),
                html.escape(list(new_texts.values())[0]),
            )
            ...
            for j in range(len(new_texts)):
                ...
                new.comparison[keys[j]] = get_text_difference(
                    html.escape(old_values[j]), html.escape(new_values[j])
                )
        elif len(old_texts) == 1:  # Old string is not pluralized, new one is
            old_values = list(old_texts.values())
            old_text = old_values[0]
            for number, text in new_texts.items():
                new.comparison[number] = get_text_difference(
                    html.escape(old_text), html.escape(text)
                )
        elif len(new_texts) == 1:  # Old string is pluralized, new one is not
            new_values = list(new_texts.values())
            new_text = new_values[0]
            new.comparison["1"] = get_text_difference(
                html.escape(old_texts["1"]), html.escape(new_text)
            )
    ...
    for admin in administrators:
        ...
        mail_context = {
            ...
            "url": request.build_absolute_uri(
                reverse("translator_request_list", args=(project.pk,))
            ),
            "settings_url": request.build_absolute_uri(reverse("user_settings")),
        }
        html_message = render_to_string(
            "traduko/email/new-translator-request.html", mail_context
        )
...
def get_last_activities(project, limit=40):
    activities = (
        StringActivity.objects.filter(trstringtext__trstring__project=project)
        .values(
            "date",
            "language__code",
            "language__name",
        ...
        .annotate(
            last=Max("datetime"),
            strings=Count("trstringtext", distinct=True),
            words_sum=Sum("words"),
        )
        .order_by("-last")[0:limit]
    )
    last_activities = {}
    for activity in activities:
...
def get_last_comments(project, limit=20):
    comments = Comment.objects.filter(trstringtext__trstring__project=project).order_by(
        "-create_date"
    )[0:limit]
    last_comments = {}
...
def send_email_about_new_comment(request, trstringtext, user):
    ...
    authors = Comment.objects.filter(
        trstringtext=trstringtext, author__email_new_comments=True
    ).exclude(author=user)
    current_language = get_language()
    authors_list = []
    for a in authors:
    ...
    for author in authors_list:
        if trstringtext.language != project.source_language or is_project_admin(
            author, project
        ):
            url = request.build_absolute_uri(
                reverse("translate", args=(project.pk, trstringtext.language.code))
            )
        else:
            ...
            if lang.count() == 0:
                continue
            url = request.build_absolute_uri(
                reverse("translate", args=(project.pk, lang[0].language.code))
            )
        ...
        html_message = render_to_string(
            "traduko/email/comment-reply.html", mail_context
        )
        plain_text_message = strip_tags(html_message)
