...
def get_strings(request):
    ...
    chosen_string_id = (
        postdata["chosen_string"] if "chosen_string" in postdata.keys() else None
    )
    previous_ids = postdata["previous_ids"] if "previous_ids" in postdata.keys() else []

    all_strings = get_all_strings(
        current_project, current_language, state_filter, search_string
    )
    strings, can_load_more = get_strings_to_translate(
        all_strings,
        current_language,
        current_directory,
        sort,
...
def get_string_translation(request):
    postdata = json.loads(request.body.decode("utf-8"))
    translated_text = get_object_or_404(
        TrStringText, language=postdata["language"], trstring=postdata["trstring_id"]
    )
...
def change_translation_state(request, trstringtext_id, state):
    ...
    current_string.original_text = TrStringText.objects.get(
        language=current_string.project.source_language, trstring=current_string
    )
    ...
    update_translators_when_translating(
        request.user, current_string.project, trstringtext.language
    )
...
def markoutdated(request):
    postdata = json.loads(request.body.decode("utf-8"))
    return change_translation_state(
        request, postdata["trstringtext_id"], TRANSLATION_STATE_OUTDATED
    )
...
def marktranslated(request):
    postdata = json.loads(request.body.decode("utf-8"))
    return change_translation_state(
        request, postdata["trstringtext_id"], TRANSLATION_STATE_TRANSLATED
    )
...
def save_translation(request):
    ...
    if editmode:
        ...
        original_text = TrStringText.objects.get(
            language=current_string.project.source_language, trstring=current_string
        )
...
def add_string(request):
    ...
    saved_string = saved_data["trstring"].to_dict(
        saved_data["trstringtext"], saved_data["trstringtext"]
    )
...
def get_history(request):
    ...
    old_versions = TrStringTextHistory.objects.filter(
        trstringtext=trstringtext
    ...
    history.insert(
        0,
        TrStringTextHistory(
            text=trstringtext.text,  # Add the current version to the beginning in order to show comparisons
...
def delete_comment(request):
    ...
    if (
        is_project_admin(request.user, comment.trstringtext.trstring.project)
...
def get_translation_suggestions(request):
    ...
    trstringtext = get_object_or_404(
        TrStringText, trstring=trstring, language=trstring.project.source_language
    )
    current_language = get_object_or_404(Language, code=postdata["language"])

    same_texts = TrStringText.objects.filter(
        language=trstring.project.source_language, text=trstringtext.text
    ).exclude(pk=trstringtext.pk)
    trstring_ids = [s.trstring.pk for s in same_texts]
    possible_translations = TrStringText.objects.filter(
        language=current_language, trstring__in=trstring_ids
    )
