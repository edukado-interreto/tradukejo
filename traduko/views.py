...
def projects(request):
    if request.user.is_superuser:
        ...
        projects_list = Project.objects.filter(
            Q(visible=True) | Q(admins=request.user)
        ).distinct()
    else:
...
def projectpage(request, project_id):
    ...
    if project_admin:
        if current_project.last_translator_notification is None:
            ...
            new_texts = TrString.objects.filter(
                project=current_project,
                last_change__gte=current_project.last_translator_notification,
            ).count()
            show_translator_notifications_button = new_texts > 0
    else:
        show_translator_notifications_button = False

    languages_with_stats = get_project_language_statistics(
        current_project, request.user
    )
    ...
    if user_is_project_admin:
        context["translator_request_count"] = TranslatorRequest.objects.filter(
            language_version__project=current_project
        ).count()
...
def translate(request, project_id, language=""):
    ...
    js_constants = {
        "projectId": current_project.pk,
        "csrf": csrf.get_token(request),
        "baseURL": reverse(
            "translate_without_language", kwargs={"project_id": current_project.pk}
        ),
        "imgURL": static("traduko/img"),
        "currentLocale": request.LANGUAGE_CODE,
        "URLprefix": f"/{request.LANGUAGE_CODE}",
        "projectLanguage": current_project.source_language.code,
        "availableLanguages": [l.to_dict() for l in available_languages],
        "userId": request.user.id,
        "isAdmin": is_project_admin(request.user, current_project),
        "globals": {
